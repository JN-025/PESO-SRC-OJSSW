...
if (isset($_POST["submit"])) {
    ...
    if ($password === $confirm_password) {
        ...
        $dolePermitCaseImg = $_FILES['dolepermitcase_img'];

        $uploadFolder = "requirement_storage/";

        if ($profileImg['error'] === 0 &&
            $dolePermitImg['error'] === 0 &&
            $listClientsImg['error'] === 0 &&
            $jobOpeningImg['error'] === 0 &&
            $dolePermitCaseImg['error'] === 0) {

            move_uploaded_file($profileImg['tmp_name'], $uploadFolder . $profileImg['name']);
            move_uploaded_file($dolePermitImg['tmp_name'], $uploadFolder . $dolePermitImg['name']);
            move_uploaded_file($listClientsImg['tmp_name'], $uploadFolder . $listClientsImg['name']);
            move_uploaded_file($jobOpeningImg['tmp_name'], $uploadFolder . $jobOpeningImg['name']);
            move_uploaded_file($dolePermitCaseImg['tmp_name'], $uploadFolder . $dolePermitCaseImg['name']);

            $profilePath = $uploadFolder . $profileImg['name'];
            $dolePermitPath = $uploadFolder . $dolePermitImg['name'];
            $listClientsPath = $uploadFolder . $listClientsImg['name'];
