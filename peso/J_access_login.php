...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "SELECT * FROM access_account WHERE email='$email' AND type = 'Job Posting' AND password='$password'";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) === 1) {
            $row = mysqli_fetch_assoc($result);
            $_SESSION['access_id'] = $row['access_id'];
            header('location: jobposting_homepage.php');
        } else {
        ...
        if(isset($_POST['signin'])){
            ...
            if(count(fetchAll($query)) > 0){ //this is to catch unknown error.
                  foreach(fetchAll($query) as $row){
                    if($row['email']==$email&&$row['password']==$password&&$row['type']=='Job Posting'){
                        $_SESSION['login'] = true;
