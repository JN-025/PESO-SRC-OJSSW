...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "SELECT * FROM access_account WHERE email='$email' AND type = 'Other Services' AND password='$password'";
        ...
        if(isset($_POST['signin'])){
            ...
            if(count(fetchAll($query)) > 0){ //this is to catch unknown error.
                  foreach(fetchAll($query) as $row){
                    if($row['email']==$email&&$row['password']==$password&&$row['type']=='Other Services'){
                        $_SESSION['login'] = true;
